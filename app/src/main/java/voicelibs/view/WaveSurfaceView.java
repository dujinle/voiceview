...
public class WaveSurfaceView extends SurfaceView implements SurfaceHolder.Callback{
	...
	private EchoActivity controler;
	...
	public void setMainControler(EchoActivity controler){
    ...
    public  void initSurfaceView( final SurfaceView sfv){
    	new Thread(){
    		public void run() {
				...
				if (canvas == null) {
					Log.i("main", "start initSurfaceView get Canvas null");
					return;
				}
...
//    	         canvas.drawLine(0, height*0.75f+20, sfv.getWidth(),height*0.75f+20, paintLine);//第3根线
				...
				Log.i("main", "start initSurfaceView finish");
				if(((WaveSurfaceView)sfv).getFilename() != null) {
					controler.startDraw(sfv, ((WaveSurfaceView) sfv).getFilename());
				}
    		};
    	}.start();
